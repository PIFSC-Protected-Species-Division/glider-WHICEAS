function pp = workflow_downloadScript_sg274_20260128_WHICEAS(configFile, preload)
% WORKFLOW_DOWNLOADSCRIPT_SG274_20260128_WHICEAS.M
...
% ppBasic = pp(:, 1:7);
% ppBasic.startLat = cellfun(@(x) x(1), ppBasic.startGPS);
% ppBasic.startLon = cellfun(@(x) x(2), ppBasic.startGPS);
% ppBasic.endLat   = cellfun(@(x) x(1), ppBasic.endGPS);
% ppBasic.endLon   = cellfun(@(x) x(2), ppBasic.endGPS);
% ppBasic.startGPS = [];
% ppBasic.endGPS   = [];
% 
