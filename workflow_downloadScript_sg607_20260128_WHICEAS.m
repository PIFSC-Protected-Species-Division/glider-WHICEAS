function pp = workflow_downloadScript_sg607_20260128_WHICEAS(preload)
...
% initialize agate
CONFIG = agate('C:\Users\selene.fregosi\Desktop\sg607_20260128_WHICEAS\agate_config_sg607_20260128_WHICEAS.cnf');
...
% % make simple version for Aaron's map
% ppBasic = pp(:, 1:5);
% 
% % Preallocate with NaNs
% n = height(ppBasic);
% ppBasic.startLat = NaN(n,1);
% ppBasic.startLon = NaN(n,1);
% ppBasic.endLat   = NaN(n,1);
% ppBasic.endLon   = NaN(n,1);
% 
% % Identify valid GPS entries
% hasStartGPS = cellfun(@(x) isnumeric(x) && numel(x) >= 2, ppBasic.startGPS);
% hasEndGPS   = cellfun(@(x) isnumeric(x) && numel(x) >= 2, ppBasic.endGPS);
% 
% % Safely extract values
% ppBasic.startLat(hasStartGPS) = cellfun(@(x) x(1), ppBasic.startGPS(hasStartGPS));
% ppBasic.startLon(hasStartGPS) = cellfun(@(x) x(2), ppBasic.startGPS(hasStartGPS));
% 
% ppBasic.endLat(hasEndGPS) = cellfun(@(x) x(1), ppBasic.endGPS(hasEndGPS));
% ppBasic.endLon(hasEndGPS) = cellfun(@(x) x(2), ppBasic.endGPS(hasEndGPS));
% 
% % Remove original cell columns
% ppBasic.startGPS = [];
% ppBasic.endGPS   = [];
% writetable(ppBasic, fullfile('A:\My Drive\Glider Rodeo Logistics - 2026 Hawaii', ...
%     'Seaglider Dive Tables', ['diveTracking_' CONFIG.glider '.csv']));
